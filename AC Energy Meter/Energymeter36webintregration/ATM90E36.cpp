...
double ATM90E36::GetImportEnergy() {
  ...
  return (double)(((double)ienergyT * 0.01) / 3200);  // returns kWh
}
...
double ATM90E36::GetExportEnergy() {
  ...
  return  (double)(((double)eenergyT * 0.01) / 3200); // returns kWh 
}
...
void ATM90E36::begin(int pin, unsigned short lineFreq, unsigned short pgagain, unsigned short ugain, unsigned short igainA, unsigned short igainB, unsigned short igainC, unsigned short igainN)
{
  ...
  CommEnergyIC(WRITE, GainA, 0xFDE0);       // Line calibration gain
  CommEnergyIC(WRITE, PhiA, 0x9C);        // Line calibration angle
  CommEnergyIC(WRITE, GainB, 0xFDE0);       // Line calibration gain
  CommEnergyIC(WRITE, PhiB, 0x9C);        // Line calibration angle
  CommEnergyIC(WRITE, GainC, 0xFDE0);       // Line calibration gain
  CommEnergyIC(WRITE, PhiC, 0x9C);        // Line calibration angle
  CommEnergyIC(WRITE, PoffsetA, 0x0100);    // A line active power offset
  CommEnergyIC(WRITE, QoffsetA, 0x0100);    // A line reactive power offset
  CommEnergyIC(WRITE, PoffsetB, 0x0100);    // B line active power offset
  CommEnergyIC(WRITE, QoffsetB, 0x0100);    // B line reactive power offset
  CommEnergyIC(WRITE, PoffsetC, 0x0100);    // C line active power offset
  CommEnergyIC(WRITE, QoffsetC, 0x0100);    // C line reactive power offset
  ...
  CommEnergyIC(WRITE, UoffsetA, 0x8500);    // A Voltage offset
  CommEnergyIC(WRITE, IoffsetA, 0x8300);    // A line current offset
  CommEnergyIC(WRITE, UgainB, _ugain);      // B Voltage rms gain
  CommEnergyIC(WRITE, IgainB, _igainB);      // B line current gain
  CommEnergyIC(WRITE, UoffsetB, 0x8500);    // B Voltage offset
  CommEnergyIC(WRITE, IoffsetB, 0x8300);    // B line current offset
  CommEnergyIC(WRITE, UgainC, _ugain);      // C Voltage rms gain
  CommEnergyIC(WRITE, IgainC, _igainC);      // C line current gain
  CommEnergyIC(WRITE, UoffsetC, 0x8500);    // C Voltage offset
  CommEnergyIC(WRITE, IoffsetC, 0x8300);    // C line current offset
